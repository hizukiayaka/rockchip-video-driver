...
------------------------------------------------------------------------------*/
u32 h264bsdDecode(struct rk_avc_decoder *dec,
		decContainer_t * pDecCont, const u8 * byteStrm, u32 len,
...
/* Code */
    ...
    if(accessUnitBoundaryFlag)
    {
        ...
        if(0)//(pStorage->picStarted && pStorage->activeSps != NULL)
        {
            ...
            if (!pStorage->validSliceInAccessUnit)
            {
                ...
                {
                    //printf("h264bsdAllocateDpbImage in h264bsdDecode 0\n");
    ...
    case NAL_SUBSET_SEQ_PARAM_SET: {
		...
		dec->width = 16 * seqParamSet->picWidthInMbs;
		dec->height = 16 * seqParamSet->picHeightInMbs;
    ...
    case NAL_CODED_SLICE_EXT: {
        ...
        if (h264bsdIsStartOfPicture(pStorage))
        {
            ...
            else
            {
				...
				{
					dpbStorage_t *dpb = pStorage->dpb;	//b 3?¨º¡À¨¨???¨®D¨°???,?¨°¨¨??a3?¡ä¨ª;
                ...
                switch (picStructCurr) {
	                ...
	                case TOPFIELD : {
						...
						if(!needCheck && pStorage->dpb->fieldmark == 1)	//clear only one field, I slice ?¨²errorconceal?¨¢¡À?¡ä|¨¤¨ª
						{
							markErrorDpbSlot(pStorage->dpb);
						}
	                ...
	                case BOTFIELD : {
	                    if (picStructPrev == BOTFIELD) {
	                        ...
	                        needCheck   = 1;
	                        pStorage->dpb->fieldmark = 0;
							break;	//?a¨¤?¨º??a¦Ì?¦Ì¡À?¡ã¦Ì?bottom field
	                    }
                ...
                if (err) {
                    ...
                    //printf("Second field missing...");
                    *readBytes = 0;
                    //pStorage->prevBufNotFinished = HANTRO_TRUE;
                    //pStorage->checkedAub = 1;
                    //printf("found unpaired field\n");
				...
				if (((diff > 2) || (diff < -2)) && ((pSliceCurr->sliceType != (I_SLICE+5)) && (pSliceCurr->sliceType != I_SLICE))
				...
				if(((pSliceCurr->sliceType == (I_SLICE+5)) || (pSliceCurr->sliceType == I_SLICE))
							&& (!IS_IDR_NAL_UNIT(&nalUnit)) && ((diff > 2) || (diff < -2)) && pDecCont->ts_en)
				{
