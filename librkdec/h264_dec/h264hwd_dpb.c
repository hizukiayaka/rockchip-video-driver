...
static void SetPoc(dpbPicture_t *pic, const i32 *poc, const u32 f)
{
    ...
    {
        #if SHOW_DPB
            printf("field set poc %d ts high %d low %d\n", poc[f], pic->TimeHigh, pic->TimeLow);
        #endif
    ...
    {
        #if SHOW_DPB
            printf("frame set poc %d %d ts high %d low %d\n", poc[0], poc[1], pic->TimeHigh, pic->TimeLow);
        #endif
...
dpbMem_t* newDpbMem(dpbStorage_t *dpb)
{
    ...
    if (p) {
        ...
        if (0 == VPUMallocLinear(&p->mem, dpb->picbuffsie)) {
            ...
            list_add_tail(&p->list, &dpb->memListFree);
            return p;
        }
        free(p);
    }
...
dpbMem_t* getDpbMem(dpbStorage_t *dpb)
{
    struct list_head *list = NULL;
    dpbMem_t *mem = NULL;
    if (list_empty(&dpb->memListFree)) {
    	printf("--------------------getDpbMem can not find free memory slot\n");
    	ASSERT(0);
	return NULL;
    }
    list = dpb->memListFree.next;
    list_del_init(list);
    mem = list_entry(list, dpbMem_t, list);
    list_add_tail(list, &dpb->memListUsed);
    //printf("+++++++++++++ getDpbMem  buffer index %d\n", mem->buffer_index);

    return mem;
}

void putDpbMem(dpbStorage_t *dpb, dpbMem_t* p)
{
    struct list_head *list = &p->list;
    dpbMem_t *mem = list_entry(list, dpbMem_t, list);;

    //printf("------------- putDpbMem  buffer index %d\n", mem->buffer_index);
    p->memStatus &= ~MEM_DPB_USE;
    p->memStatus |= MEM_DPBDONE;
    list_del_init(list);
    list_add_tail(list, &dpb->memListFree);
}

i32 freeDpbMem(dpbMem_t* p)
{
    printf("************* freeDpbMem buffer index %d\n", p->buffer_index);
...
i32 freeDpbMemFromDpb(dpbStorage_t *dpb, dpbMem_t* p)
{
    putDpbMem(dpb, p);
    return 0;
}
...
dpbPicture_t* newDpbSlot(dpbStorage_t *dpb)
{
    ...
    if (!list_empty(&dpb->dpbFree))
    {
        ...
        struct list_head unused_list = p->unused_list;
        list_del_init(&p->list);
        memset(p, 0, sizeof(dpbPicture_t));
        p->unused_list = unused_list;
        return p;
    } else {
...
i32 freeDpbSlot(dpbStorage_t *dpb, dpbPicture_t** p)
{
    ...
    if (!p || !(ptr = *p)) {
        printf("freeDpbSlot invalid parameter ptr is %p\n", p);
        return -1;
    }
    ...
    list_add_tail(&ptr->unused_list, &dpb->dpbUnused);
    
    *p = NULL;
    return 0;
}

i32 getUnusedDpb(dpbStorage_t *dpb)
{
    dpbPicture_t *tmp;

    if (!list_empty(&dpb->dpbUnused)) {
        tmp = list_entry(dpb->dpbUnused.next, dpbPicture_t, unused_list);
        DPBDEBUG("%d %d frame num %d\n", tmp->buffer_index, tmp->data->buffer_index, tmp->frameNum);

        list_del_init(&tmp->unused_list);
        return tmp->data->buffer_index;
    }

    DPBDEBUG("no unused index found\n");
    return -1;
}
...
{
    if (!p) {
        printf("freeOutSlot invalid zero parameter\n");
        return -1;
    }
...
i32 markErrorDpbSlot(dpbStorage_t *dpb)
{
    ...
    //printf("markErrorDpbSlot");
    if (dpb->currentOut) {
        //printf("clear currentOut  idx %d", dpb->currentOut->dpbIdx);
...
/* Code */
    ...
    {
        #if SHOW_DPB
            printf("mark poc %d reference picture\n", *picOrderCnt);
        #endif
        ...
        {
            #if SHOW_DPB
                printf("SlidingWindowRefPicMarking\n");
            #endif
        ...
        if(!markedAsLongTerm)
        {
            ...
            /*else
            {
					...
					printf("no room for buffer numRefFrames %d maxRefFrames %d secondField %d\n", dpb->numRefFrames, dpb->maxRefFrames, secondField);
                #endif
                status = HANTRO_NOK;
            }*/
        }
...
void h264DpbTryClearSlot(dpbStorage_t * dpb)
{
    ...
    for (i = 0; i <= dpb->dpbSize; i++) {
        if (dpb->buffer[i]) {
            ...
            if (!IS_EXISTING(dpbPic, FRAME)) {
                ...
                freeDpbMemFromDpb(dpb, dpbPic->data);
                DPBDEBUG("freemem dpb->buffer[%d]=%p, poc=%d, %d", i, dpb->buffer[i], dpbPic->picOrderCnt[0], dpbPic->picOrderCnt[1]);
    ...
    if (!clear) {
        ...
        //printf("SlidingWindowRefPicMarking ret %d", clear);
    }
}
...
void h264DpbUpdateOutputList(dpbStorage_t * dpb, u32 picStruct)
{
    ...
    if ((picStruct != BOTFIELD) || !find) {//todo
        u32 i, loop = 0;
        u32 found = 0;
        do {
            found = 0;
...
void h264bsdDpbDump(dpbStorage_t * dpb)
{
    ...
    dpbPicture_t *ret = NULL;
    list_for_each_entry(tmp, &dpb->dpbList, list) {
        printf("normal list: dpbIdx %d picNum %d frameNum %d poc %d toBeDisplayed %d IS_REFERENCE_F %d\n",
            tmp->dpbIdx, tmp->picNum, tmp->frameNum, GET_POC(tmp), tmp->toBeDisplayed, IS_REFERENCE_F(tmp));
    }
    if (dpb->currentOut) {
        tmp = dpb->currentOut;
        printf("currentOut : dpbIdx %d picNum %d frameNum %d poc %d toBeDisplayed %d IS_REFERENCE_F %d\n",
            tmp->dpbIdx, tmp->picNum, tmp->frameNum, GET_POC(tmp), tmp->toBeDisplayed, IS_REFERENCE_F(tmp));
    }
    if (dpb->previousOut) {
       tmp = dpb->previousOut;
       printf("previousOut: dpbIdx %d picNum %d frameNum %d poc %d toBeDisplayed %d IS_REFERENCE_F %d\n",
       tmp->dpbIdx, tmp->picNum, tmp->frameNum, GET_POC(tmp), tmp->toBeDisplayed, IS_REFERENCE_F(tmp));
    }
    printf("lastPicOrderCnt %d\n", dpb->lastPicOrderCnt);
...
void *h264bsdAllocateDpbImage(storage_t * pStorage, dpbStorage_t * dpb)
{
    ...
    if (NULL == pMem) {
        printf("h264bsdAllocateDpbImage failed");
        return NULL;
    }

    if (dpb->currentOut) {
        printf("h264bsdAllocateDpbImage found currentOut is not NULL\n");
    }
...
/* Code */
    ...
    INIT_LIST_HEAD(&dpb->memListUsed);
    INIT_LIST_HEAD(&dpb->memListFree);
    INIT_LIST_HEAD(&dpb->dpbList);
    INIT_LIST_HEAD(&dpb->dpbFree);
    INIT_LIST_HEAD(&dpb->dpbUnused);
    ...
    dpb->buffer_num = 20;
    #if SHOW_DPB
        printf("h264bsdInitDpb: dpbSize %d totBuffers %d \n", dpbSize, dpb->buffer_num);
    #endif

    for(i = 0; i < dpb->buffer_num; i++)
    {
    	dpbMem_t* mem = newDpbMem(dpb);
	mem->buffer_index = i;
    }

    memset(dpb->buffer, 0, sizeof(dpb->buffer));
    for (i = 0; i <= dpbSize; i++) {
...
/* Code */
    ...
    //printf("h264bsdCheckGapsInFrameNum");
...
/* Code */
...
#if SHOW_SMALL_POC
    printf("OutputPicture fullness %d, dpbSize %d flush %d, numRefFrames %d\n", dpb->fullness, dpb->dpbSize, flush, dpb->numRefFrames);
    h264bsdDpbDump(dpb);
#endif
    ...
    if (NULL == tmp)
    {
        printf("OutputPicture ret NULL %d\n", last_no_out);
        if (last_no_out)
        {
            for(i = 0; i <= dpb->dpbSize; i++)
            {
                printf("%2d disp %d ref %d poc %d last %d\n",
                    i,
    ...
    {
        last_no_out = 0;
        printf("OutputPicture get poc %d last %d\n", GET_POC(*tmp), dpb->lastPicOrderCnt);
    }
#endif
...
/* Code */
    ...
    if (NULL == dpbOut)
    {
    	printf("OutputPicture# newOutSlot fail");
        return HANTRO_NOK;
    }
    dpbOut->data = tmp->data;
    dpbOut->isIdr = tmp->isIdr;
	//dpbOut->isItype = tmp->isItype;
    ...
    {
        printf("find olny one field output! poc0=%d, poc1=%d", tmp->picOrderCnt[0], tmp->picOrderCnt[1]);
    ...
    {
        freeDpbMemFromDpb(dpb, tmp->data);
...
/* Code */
...
#if SHOW_DPB
    printf("h264bsdDpbOutputPicture numOut %d\n", dpb->numOut);
#endif
    if (dpb->numOut && !list_empty(&dpb->outList)) {
		...
		if((pOut->h264hwStatus[0]&0x10000) && (!pOut->isFieldPic) && dpb->ts_en==3)
		{
			OutBufFree(dpb, pOut);
...
#endif
		{
            ...
            pOutput->buffer_index = pOut->data->buffer_index;
			...
			//pOutput->isItype = pOut->isItype;
            pOutput->interlaced = pOut->interlaced;
            //pOutput->fieldOrder = pOut->fieldOrder;
...
#if SHOW_DPB
    printf("DpbBufFree poc %d\n", GET_POC(p));
#endif

    if (!p->toBeDisplayed)
    {
    	DPBDEBUG("DpbBufFree poc=%d\n", GET_POC(p));
        freeDpbMemFromDpb(dpb, p->data);
...
void h264decErrorConCealMent(decContainer_t * pDecCont)
{
    ...
    if (pStorage->sliceHeader->fieldPicFlag == 0){
		...
		//printf("pDecCont->h264hwStatus[%d]=0x%x", picStruct, pDecCont->h264hwStatus[picStruct]);
	}
