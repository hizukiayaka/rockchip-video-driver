...
printf("~~ AllocateAsicBuffers, support:%d\n", pDecCont->h264ProfileSupport);
...
/***************************************************************************/
	...
	memcpy(h264regtmp, pDecCont->h264Regs, sizeof(h264regtmp));
	    ...
	    //printf("VPUClientWaitResult: ret %d, cmd %d, len %d\n", ret, cmd, len);
		if(pSliceHeader->fieldPicFlag){
...
static void H264FlushRegs(decContainer_t * pDecCont)
{
    //printf("H264FlushRegs\n");

    /*if (VPUClientSendReg(pDecCont->socket, pDecCont->h264Regs, DEC_X170_REGISTERS)) {
        printf("H264FlushRegs fail\n");
    } else {
        printf("H264FlushRegs success\n");
    }*/
}
...
void H264InitRefPicList(decContainer_t * pDecCont)
{
	...
	memcpy(dpb->list0,list0,sizeof(list0));
	memcpy(dpb->list1,list1,sizeof(list1));
	memcpy(dpb->listP,listP,sizeof(listP));
